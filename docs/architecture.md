...
- **`[include(틀:...)]` pre-processing** (`src/workers/includeTemplates.ts`): namumark는 include를 자리표시 링크로만 출력하므로 워커에서 직접 처리. 파싱 전 known 틀 호출을 PUA 토큰(U+E025/U+E026)으로 치환 → namumark 파싱 → HTML에서 토큰을 실제 박스 HTML로 복원. unknown 틀은 원문 그대로 유지해 기존 CSS 숨김(`a[href^="/w/%ED%8B%80%3A"]`) 적용. 지원 틀: `틀:상세 내용`, `틀:상위 문서`, `틀:하위 문서`, `틀:관련 문서`, `틀:다른 뜻`/`틀:다른 뜻1`, `틀:네모틀`/`틀:네모틀b` (인라인 테두리 박스), `틀:글배경`/`틀:글배경b`/`틀:글배경r`/`틀:글배경br` (인라인 배경색), `틀:날짜 이동` (날짜 문서 네비게이션). 파라미터 체계: `틀:다른 뜻`은 `설명N`/`문서명N`, `틀:다른 뜻1`은 `otherN`/`rdN` (병행 지원). 색상·CSS 값 파라미터는 `sanitizeColor`/`sanitizeCSSValue`로 화이트리스트 검증 후 인라인 스타일 삽입. CSS 클래스: 블록 네비게이션 박스 `.namu-include`, 인라인 테두리 `.namu-box`, 인라인 배경색 `.namu-bg`, 날짜 이동 테이블 `.namu-date-nav`.
