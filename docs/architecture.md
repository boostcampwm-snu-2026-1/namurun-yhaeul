...
- **콘텐츠 영역 레이아웃**: `overflow-y-auto pr-14` div 내부를 `flex` row로 구성 — 아티클 래퍼(`flex-1 min-w-0 relative`)만 배치. `ArticleNavButtons`는 거터 div 없이 GamePage 루트에서 `fixed right-4 bottom-4 z-30`으로 렌더링. `pr-14`로 버튼 뒤에 콘텐츠가 가려지지 않도록 우측 여백 확보. 스크롤 애니메이션은 `requestAnimationFrame` + easeInOutCubic으로 직접 구현(`smoothScroll` 함수). `hasToc` state: `handleArticleReady` 시점에 `contentRef.current?.querySelector('.opennamu_TOC')`로 TOC 유무를 감지해 `ArticleNavButtons`에 전달
