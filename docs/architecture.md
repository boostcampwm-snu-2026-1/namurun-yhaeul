...
- **`[include(틀:...)]` pre-processing** (`src/workers/includeTemplates.ts`): namumark는 include를 자리표시 링크로만 출력하므로 워커에서 직접 처리. 파싱 전 known 틀 호출을 PUA 토큰(U+E025/U+E026)으로 치환 → namumark 파싱 → HTML에서 토큰을 실제 박스 HTML로 복원. unknown 틀은 원문 그대로 유지해 기존 CSS 숨김(`a[href^="/w/%ED%8B%80%3A"]`) 적용. 지원 틀: `틀:상세 내용`, `틀:상위 문서`, `틀:하위 문서`, `틀:관련 문서`, `틀:다른 뜻`/`틀:다른 뜻1`. 파라미터 체계: `틀:다른 뜻`은 `설명N`/`문서명N`, `틀:다른 뜻1`은 `otherN`/`rdN` (병행 지원). 렌더링 결과는 `.namu-include` CSS 클래스로 스타일링.
